...
<head>
    ...
    <title>Edit Book — BookMart</title>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .page-wrap { display:grid; grid-template-columns:1fr 420px; gap:0; max-width:1200px; margin:0 auto; padding:40px 24px; }
        ...
        /* Current image */
        .current-img-wrap { margin-bottom:16px; }
        .current-img { width:100%; max-height:200px; object-fit:cover; border-radius:10px; border:1px solid var(--border); }
        .current-img-label { font-size:12px; color:var(--text-muted); margin-bottom:8px; }

        .upload-zone { border:1.5px dashed var(--border); border-radius:12px; padding:24px; text-align:center; cursor:pointer; transition:all 0.2s; position:relative; background:var(--surface-2); }
        ...
        .card-title { font-size:15px; font-weight:700; color:var(--text-primary); margin-bottom:4px; min-height:22px; }
        .card-author { font-size:12px; color:var(--text-secondary); margin-bottom:10px; }
        .card-meta { display:flex; align-items:center; justify-content:space-between; margin-bottom:12px; }
        .card-category { font-size:11px; color:var(--text-muted); background:var(--surface-2); border:1px solid var(--border); padding:3px 9px; border-radius:6px; }
        .card-price { font-size:20px; font-weight:800; color:#a78bfa; }
        .card-desc { font-size:12px; color:var(--text-secondary); line-height:1.6; border-top:1px solid var(--border); padding-top:12px; }
...
<div class="page-wrap">
    <div class="form-side">
        ...
        <div class="breadcrumb">
            ...
            <a href="{{ route('books.show', $book) }}">{{ Str::limit($book->title, 20) }}</a> ›
            <span style="color:var(--text-secondary);">Edit</span>
        </div>

        <div class="page-title">✏️ Edit listing</div>
        <div class="page-sub">Update your book details — preview updates live →</div>

        @if(session('success'))
            <div class="alert-success">✅ {{ session('success') }}</div>
        @endif

        <form method="POST" action="{{ route('books.update', $book) }}" enctype="multipart/form-data" id="bookForm">
            @csrf
            @method('PUT')
            ...
            <div class="section">
                ...
                <div class="field">
                    ...
                    <input type="text" name="title" id="f-title" value="{{ old('title', $book->title) }}" placeholder="Book title" required>
                    @error('title')<div class="error-msg">⚠️ {{ $message }}</div>@enderror
                </div>
                <div class="field">
                    <label>Author <span class="req">*</span></label>
                    <input type="text" name="author" id="f-author" value="{{ old('author', $book->author) }}" placeholder="Author name" required>
                ...
                <div class="field">
                    ...
                    <textarea name="description" id="f-desc" placeholder="Describe the book...">{{ old('description', $book->description) }}</textarea>
                    <div style="font-size:11px;color:var(--text-muted);margin-top:4px;text-align:right;"><span id="descCount">0</span>/500</div>
                ...
                <div class="field-row">
                    <div class="field">
                        ...
                        <div class="input-prefix-wrap">
                            ...
                            <input type="number" name="price" id="f-price" value="{{ old('price', $book->price) }}" min="0" step="1" required>
                        </div>
                    ...
                    <div class="condition-pills">
                        ...
                        <div class="condition-pill">
                            <input type="radio" name="condition" id="cond-{{ $loop->index }}" value="{{ $cond }}"
                                {{ old('condition', $book->condition) == $cond ? 'checked' : '' }}
                                onchange="updateCondition('{{ $cond }}')">
            ...
            <div class="section">
                <div class="section-header">
                    ...
                    <div class="section-sub">Optional — keep current or upload new</div>
                </div>

                @if($book->image)
                    <div class="current-img-wrap">
                        <div class="current-img-label">📸 Current image:</div>
                        <img src="{{ Storage::url($book->image) }}" alt="{{ $book->title }}" class="current-img" id="cardImgEl">
                    </div>
                @endif

                <div class="upload-zone">
                    <input type="file" name="image" accept="image/*" onchange="handleImage(event)">
                    <div class="upload-icon">📷</div>
                    <div class="upload-title">Upload new image</div>
                    <div class="upload-sub">or <strong>click to browse</strong> — PNG, JPG up to 2MB</div>
    ...
    <div class="preview-side">
        ...
        <div class="buyer-card">
            <div class="card-img" id="cardImgWrap">
                @if($book->image)
                    <img src="{{ Storage::url($book->image) }}" alt="{{ $book->title }}" id="cardImgPreview">
                @else
                    <span id="cardImgPlaceholder">📖</span>
                @endif
                <span class="card-condition-badge" id="condBadge" style="background:rgba(63,185,80,0.2);color:#3fb950;">{{ $book->condition }}</span>
            </div>
            <div class="card-body">
                <div class="card-title" id="prev-title">{{ $book->title }}</div>
                <div class="card-author" id="prev-author">{{ $book->author }}</div>
                <div class="card-meta">
                    <span class="card-category" id="prev-category">{{ $book->category->name ?? 'Category' }}</span>
                    <span class="card-price" id="prev-price">৳{{ number_format($book->price,0) }}</span>
                </div>
                <div class="card-desc" id="prev-desc">{{ $book->description ?? 'No description.' }}</div>
            </div>
        </div>
    </div>
</div>
...
document.getElementById('f-title').addEventListener('input', e => {
    document.getElementById('prev-title').textContent = e.target.value || 'Book title will appear here';
});
document.getElementById('f-author').addEventListener('input', e => {
    document.getElementById('prev-author').textContent = e.target.value || 'Author name';
});
document.getElementById('f-price').addEventListener('input', e => {
    document.getElementById('prev-price').textContent = e.target.value ? '৳' + parseInt(e.target.value).toLocaleString() : '৳ —';
});
document.getElementById('f-desc').addEventListener('input', e => {
    document.getElementById('prev-desc').textContent = e.target.value || 'Description will appear here...';
...
document.getElementById('f-category').addEventListener('change', e => {
    ...
    document.getElementById('prev-category').textContent = opt.text || 'Category';
});
...
function updateCondition(cond) {
    ...
    badge.style.cssText = colors[cond] || colors['New'];
...
function handleImage(event) {
    ...
    reader.onload = e => {
        const preview = document.getElementById('cardImgPreview') || document.getElementById('cardImgEl');
        if (preview) {
